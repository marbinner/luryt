# The Optimized Conlang – Full Foundational Spec (with Voice Series)
...
1. **Atomic words**
   ...
   * Closed classes: pronouns, particles, prepositions, auxiliaries, digits, wh-words, quantifiers.
...
(Some forms, like *Va* and *Vi*, are intentionally reused across categories, e.g. as digits; context disambiguates.)
...
```text
[Quantifier/Number] [Group-Prefix + N-M(+T?)] [Adjectives] [Genitive-NP]
```
...
(Interaction of quantifiers like *kafa, kefe, nyfa* with Ko-/Ka/Ki/Ke is left flexible but generally: quantifiers and numbers precede the NP; K-series prefixes encode group/partitive semantics on the head noun.)
...
* **Particles & auxiliaries** (`jo, ju, ja, je, ba, va, vi`): `Operator`.
* **Digits** (`pa, pe, ...`): `Number`.
* **Wh-words** (`fa, fe, ...`): `Operator` (or separate `Wh` tag).
